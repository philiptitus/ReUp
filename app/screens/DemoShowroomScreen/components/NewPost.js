...
const BasicTextFields = ({ id }) => {
  ...
  const uploadFileHandler = async (uris) => {
    ...
    try {
      ...
      const { data } = await axios.post(`${API_URL}/api/v1/uploads/`, formData, config);
      ...
      // Alert.alert(
      //   "Upload Error",
      //   "The AI agent turned down your post. Please look for an environment-related post.",
      //   [{ text: "OK" }]
      // );
    }
  };
    ...
    {step === 2 && (
      <>
        <Text style={{  textAlign: 'center', marginBottom: 15 }}>
          Upload some Images To Finish Making Your Post, You Can only Upload Upto 3 Images
        </Text>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 20 }}>
          ...
          >
            <Text style={{  fontSize: 16 }}>Discard Post</Text>
          </TouchableOpacity>
...
const BasicVideoFields = ({ id }) => {
  ...
  return (
    ...
    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', padding: 20 , width: screenWidth2}}>
      <View style={{ width: '100%', padding: 20, borderRadius: 10 }}>
        {stepper === 1 && (
          <>
            <TextInput
        ...
        {postType === null && (
          <View style={styles.postTypeSelector}>
            <TouchableOpacity style={styles.button} onPress={() => handlePostTypeSelect('normal')}>
              <Text style={styles.buttonText}>Start Post</Text>
            </TouchableOpacity>
