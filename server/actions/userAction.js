...
import { API_URL, TEST_URL } from '../constants/URL'
...
export const register = (name, email, password, userType, isDemo) => async(dispatch) => {
    try {
        dispatch({
        ...
        console.log(isDemo)
        const { data } = await axios.post(`${API_URL}/api/users/register/`, { 'name': name, 'email': email, 'password': password, 'user_type': userType , 'is_demo' : isDemo}, config);
...
export const followUser = (id) => async (dispatch, getState) => {
  ...
  export const deleteAccount = () => async (dispatch, getState) => {
    try {
        ...
        dispatch(logout());
        dispatch({ type: ACCOUNT_DELETE_RESET }); // Reset state after success
    } catch (error) {
        dispatch({
...
// export const resetAccountDelete = () => ({ type: ACCOUNT_DELETE_RESET });
